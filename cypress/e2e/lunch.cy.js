...
describe('점심 뭐 먹지 e2e 테스트', () => {
  ...
  it('헤더를 클릭하면 시작 페이지로 이동한다.', () => {
    cy.scrollTo('bottom');
    cy.get('h1').contains('점심 뭐 먹지').click();
  ...
  it('페이지에 처음 방문하면 전체 카테고리 이름순으로 정렬된 음식점 목록을 볼 수 있다. ', () => {
    ...
    cy.get('li').should('have.length', 10);
    cy.get('li').first().should('contain.text', '덮밥이맛있는집');
    cy.get('li').last().should('contain.text', '커피베네');
  });
  ...
  it('한식 카테고리를 클릭하면 한식 카테고리에 해당되는 음식점만 볼 수 있다.', () => {
    ...
    cy.get('li').first().should('contain.text', '덮밥이맛있는집');
    cy.get('li').last().should('contain.text', '정식당');
  });
  ...
  it('한식 카테고리 클릭 후 거리순으로 정렬하면 거리순으로 정렬된 음식점 목록을 볼 수 있다.', () => {
    ...
    cy.get('li').should('have.length', 5);
    cy.get('li').first().should('contain.text', '삼원가든');
    cy.get('li').last().should('contain.text', '정식당');
  });
  ...
  it('음식점 상세 정보 모달에서 웹사이트 방문하기를 누르면 음식점 웹사이트가 열린다.', () => {
    cy.get('li[data-id="7"]').click();
    cy.get('a').contains('웹사이트 방문하기').invoke('removeAttr', 'target').click({ force: true });
